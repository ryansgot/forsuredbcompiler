...
public class NewForSureGenerator extends JavaSourceGenerator {

    private final List<TableInfo> tablesSortedByName;
    private final TableInfo exampleTable;

    public NewForSureGenerator(ProcessingEnvironment processingEnv, String packageName, TableContext targetContext) {
    ...
    protected String getCode() {
        final ParameterizedTypeName infoFactoryTypeName = ParameterizedTypeName.get(ClassName.get(ForSureInfoFactory.class),
                ClassName.get(getResultParameter()),
                ClassName.get(getRecordContainer()));
    ...
    private JavadocInfo classJavadoc() {
        ...
        return JavadocInfo.builder()
                ...
                .startCode()
                .addLine("Retriever retriever = ForSure.$L().find()", snakeToCamel(exampleTable.getTableName()))
                .addLine(".byId(1)")
                .addLine(".andFinally()")
                .addLine(".get();")
                ...
                .startCode()
                .addLine("SaveResult<$L> result = ForSure.$L().find()", getResultParameter().getSimpleName(), snakeToCamel(exampleTable.getTableName()))
                .addLine(".byId(1)")
                .addLine(".andFinally()")
                .addLine(".set()")
                .addLine(".id(2)")
                ...
                .startCode()
                .addLine("SaveResult<$L> result = ForSure.$L().set()", getResultParameter().getSimpleName(), snakeToCamel(exampleTable.getTableName()))
                .addLine(".save();")
                .endCode()
                .endParagraph()
                .startParagraph()
    ...
    private void addResolverMethods(TypeSpec.Builder codeBuilder) {
        for (TableInfo table : tablesSortedByName) {
            ...
            codeBuilder.addMethod(MethodSpec.methodBuilder(snakeToCamel(table.getTableName()))
                    .addJavadoc(jd.stringToFormat(), jd.replacements())
                    .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
